...
- `@copilot review` の再依頼は「前回レビューの指摘をすべて対応後」に **1 回のみ**行う。
- 前回レビューが完了していない状態で新しいレビュー依頼を出さない。
- PR 作成後は **初回レビューおよび CI 完了を待つ（目安: 最大 15 分）**。
- 以降は **イベント駆動（CI失敗・レビューコメント追加時）で対応**する。
...
| 分類 | 基準 |
|------|------|
| `blocking`（修正必須） | 実行時エラーの可能性がある / データ整合性を破壊する / セキュリティリスク / **実行時に型エラーを引き起こす可能性がある** / **既存利用者に明確な破壊的影響がある変更** |
...
- **各レビューコメント（スレッド単位）ごとに** `accept` / `reject` を明示する。
- `reject` の場合は理由を記述する（スコープ外 / 別 PR で対応予定 / 設計方針と相違）。
- `non-blocking / suggestion` は以下のいずれかを選択する：
  - 今回対応する
  - 対応しない（理由明示）
...
- 仕様・API に変更がある
- **挙動（出力・副作用）が変化する変更がある**
- `blocking` コメントが存在した（すべて解消後に再レビュー依頼）

##### 再レビュー不要条件

- 軽微修正のみ（typo・コメント・フォーマット等）
- `non-blocking` / `suggestion` のみ対応し、**挙動に変化がない**
...
- `blocking` がすべて解消済み かつ 再レビュー必須 →
  「前回指摘がすべて解消・新たな blocking なし」を確認してから再レビュー依頼
...
- **accept したスレッドのみ `resolveReviewThread` で解決する**
- `reject` したスレッドは理由を記載し、未解決のままでもよい（議論保持のため）
- すべてのコメントには必ず返信する
...
`request_copilot_review` で Copilot に再レビューを依頼する。フォールバックとして以下のコマンドも使用可能。
...
PRコメント欄に
`<user> requested a review from Copilot`
が表示されたら成功。

再レビュー依頼後は、CI とレビューコメントを待つ。
